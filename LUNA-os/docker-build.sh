...
nohup docker run --rm \
    ...
    bash -c "
        set -e
        # 'lb clean --purge' tries to rmdir the chroot directory itself,
        # which fails since it's a tmpfs mount point ('Device or resource
        # busy'). Empty its contents instead, and clean the other
        # generated state directories (these are on the normal host
        # mount, so plain rm -rf works fine for them).
        rm -rf /build/chroot/* /build/chroot/.[!.]* 2>/dev/null || true
        rm -rf /build/.build /build/cache /build/binary* /build/*.iso 2>/dev/null || true
        lb config
        lb build
    " \
    > "$LOG_FILE" 2>&1 &

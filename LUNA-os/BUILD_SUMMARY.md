...
third-party client. Lutris isn't packaged in Debian's own repos, so it's
installed via **Flatpak/Flathub** by `0015-install-lutris-flatpak.hook.chroot`
rather than apt. Whitelisted (both as `lutris` and its Flatpak app ID
`net.lutris.Lutris`) in GameMode for automatic performance-mode switching.
...
- The kernel is `linux-image-rt-amd64` (Debian's real-time kernel), not
  the Ubuntu-only `linux-image-lowlatency` name used in an earlier draft
  of this build — Debian doesn't ship a "lowlatency" flavour.
- Lutris is installed via Flatpak (not apt) since it isn't in Debian's
  own repositories — see `0015-install-lutris-flatpak.hook.chroot`.
- RetroArch core selection is limited to what Debian's Games Team
  actually packages: Nestopia (NES), Snes9x (SNES), mGBA (GBA),
  Beetle PSX (PS1, branded "Beetle" rather than "Mednafen" in Debian's
  packaging). FBNeo, Genesis Plus GX, and Mupen64Plus-Next aren't
  currently packaged for Debian — RetroArch's own in-app core downloader
  can add them post-install if needed.
- `config/archives/luna.list.chroot` explicitly enables
  main/contrib/non-free/non-free-firmware as a belt-and-suspenders
  measure alongside `auto/config`'s `--archive-areas`, since some
  live-build versions handle that flag inconsistently.
